...
- Pi-web session rehydration no longer mistakes a replacement instance of the same session for a passive subagent, leaving Codex usage-limit errors to a different router. The replacement resumes account failover, while a superseded instance stops routing and hands off its loopback listener until the final owner exits; distinct in-process child sessions remain passive.
