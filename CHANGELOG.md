...
- **Pinned models now stay pinned during cross-account failover.** A live catalog could rank an unpinned model ahead of a model selected in `pinned-models.json`, so failover changed accounts and silently moved from Luna to Sol. Family ranking now keeps pinned models ahead while preserving live priority among multiple pins; missing or malformed pin files keep the existing order.
